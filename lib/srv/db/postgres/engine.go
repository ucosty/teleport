...
func (e *Engine) receiveFromClient(client *pgproto3.Backend, server *pgproto3.Frontend, clientErrCh chan<- error, sessionCtx *common.Session) {
	...
	for {
		...
		case *pgproto3.Query:
			e.handleQueryMessage(sessionCtx, msg)
		case *pgproto3.Parse:
			e.handleParseMessage(sessionCtx, msg)
		case *pgproto3.Bind:
			e.handleBindMessage(sessionCtx, msg)
		case *pgproto3.Execute:
			e.handleExecuteMessage(sessionCtx, msg)
		case *pgproto3.Close:
			e.handleCloseMessage(sessionCtx, msg)
		case *pgproto3.FunctionCall:
			e.handleFuncCallMessage(sessionCtx, msg)
...
// handleQueryMessage processes Query wire message which indicates that client
// is executing a simple query.
func (e *Engine) handleQueryMessage(session *common.Session, msg *pgproto3.Query) {
...
func (e *Engine) handleParseMessage(session *common.Session, msg *pgproto3.Parse) {
	e.Audit.EmitEvent(e.Context, makeParseEvent(session, msg.Name, msg.Query))
}

// handleBindMessage processes Bind wire message which readies existing prepared
...
func (e *Engine) handleBindMessage(session *common.Session, msg *pgproto3.Bind) {
...
// handleExecuteMessage processes Execute wire message which indicates that
// client is executing the previously parsed and bound prepared statement.
func (e *Engine) handleExecuteMessage(session *common.Session, msg *pgproto3.Execute) {
	e.Audit.EmitEvent(e.Context, makeExecuteEvent(session, msg.Portal))
}

// handleCloseMessage processes Close wire message which indicates that client
// is closing a prepared statement or a destination portal.
func (e *Engine) handleCloseMessage(session *common.Session, msg *pgproto3.Close) {
...
// handleFuncCallMessage processes FunctionCall wire message which indicates
// that client is executing a system function.
func (e *Engine) handleFuncCallMessage(session *common.Session, msg *pgproto3.FunctionCall) {
	var formatCodes []int16
	for _, fc := range msg.ArgFormatCodes {
		formatCodes = append(formatCodes, int16(fc))
	}
	e.Audit.EmitEvent(e.Context, makeFuncCallEvent(session, int32(msg.Function),
