...
func TestALPNProxyHTTPProxyNOProxyDial(t *testing.T) {
	...
	require.NoError(t, os.Setenv("http_proxy", u.Host))
	t.Cleanup(func() { os.Unsetenv("http_proxy") })

	require.NoError(t, os.Setenv("no_proxy", "127.0.0.1"))
	t.Cleanup(func() { os.Unsetenv("no_proxy") })
	...
	require.NoError(t, err)
	err = waitForNodeCount(context.Background(), rc, "root.example.com", 1)
	require.NoError(t, err)
	...
	require.NoError(t, err)
	err = waitForNodeCount(context.Background(), rc, "root.example.com", 2)
	require.NoError(t, err)
